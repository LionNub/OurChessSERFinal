...
public class ChessBoardPanel extends JPanel {
    ...
    */
    ...
    private void handleClick(int x, int y) {
        ...
        if (selRow < 0) {
            // IDLE -> SELECTED (only if the square has a piece) MODIFIED TO ONLY SELECT IF PLAYERS TURN
        if (clicked.getPiece() != null && clicked.getPiece().getColor() == turn) { selRow = row; selCol = col; }
			...
			statusPanel.setMessage("");  // clear previous message
			...
			statusPanel.setMessage("Illegal move");
			System.out.println("Setting message: Illegal move");
		}
        repaint();
    }
